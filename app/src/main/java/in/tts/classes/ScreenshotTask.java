...
public class ScreenshotTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected void onPreExecute() {
        ...
        CommonMethod.toDisplayToast(activity, context.getString(R.string.toast_wait_a_minute));
        ...
        try {
            windowWidth = CommonMethod.getWindowWidth(context);
            contentHeight = webView.getContentHeight() * CommonMethod.getDensity(context);
            title = HelperUnit.fileName(webView.getUrl());

        } catch (Exception e) {
            CommonMethod.toDisplayToast(activity, context.getString(R.string.toast_error));
        }
    }
    ...
    protected Boolean doInBackground(Void... params) {
        if (android.os.Build.VERSION.SDK_INT >= 23) {
            ...
            if (hasWRITE_EXTERNAL_STORAGE != PackageManager.PERMISSION_GRANTED) {
                CommonMethod.toDisplayToast(context, context.getString(R.string.toast_permission_sdCard_sec));
                HelperUnit.grantPermissionsStorage(activity);
            } else {
                try {
                    Bitmap bitmap = CommonMethod.capture(webView, windowWidth, contentHeight, Bitmap.Config.ARGB_8888);
